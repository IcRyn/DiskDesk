...
items = [(name, (root / name).read_text(encoding='utf-8')) for name in
         ('diskdesk.lua', 'diskdesk_services.lua')]
...
distribution = ('instalar_diskdesk.lua', 'diskdesk.lua', 'diskdesk_services.lua',
                'README.md', 'IDEIAS.md', 'GITHUB.md')
source = distribution + ('build_installer.py', 'installer_template.lua', '.gitignore',
                         'test_diskdesk.py', 'test_services.py', 'test_installer.py',
                         'render_preview.py')
